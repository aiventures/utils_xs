...
class ImageOrganizer:
    ...
    def _process_geo_metadata(self) -> Optional[Dict]:
        ...
        for _f_img, _img_meta in _metadata_exif.items():
            ...
            if img_geo is None and _has_osm_latlon:
                print(
                    f"{C_W}🌏 Image [{_f_img}], using OSM Coordinates {C_F}[{f_osm}]: {C_S}{img_geo_osm_lat_lon}{C_0}"
                )
    ...
    def get_image_datetime_from_exif(self, exif_metadata_image: dict) -> Optional[DateTime]:
        ...
        if datetime_s is not None:
            # fix parsing of datretime formats
            filename = exif_metadata_image.get("File", {}).get("FileName", "NO_METADATA_FOUND")
            datetime_formats = ["%Y:%m:%d %H:%M:%S.%f", "%Y:%m:%d %H:%M:%S"]
            datetime_image = None
            for dt_format in datetime_formats:
                try:
                    datetime_image = Helper.get_datetime_from_format_string(datetime_s, dt_format, timezone_)
                    printi(f"⌚ Found UTC Datetime [{datetime_image}] for image [{filename}]")
                    return datetime_image
                except ValueError:
                    continue
            printw(f"Couldn't parse datetime string for image [{filename}] [{datetime_s}], returning None")
